...
export default function HabitTriggerModal({
  habit,
  onClose,
  onConfirm,
  initialValue,
      ...
      return (
        <div className="flex flex-col items-center justify-center py-8 animate-pulse">
          ...
          <p className="text-slate-400 text-sm">
            Connecting to {habit.triggerType.toLowerCase()} service
          </p>
        </div>
      );
    }
    ...
    if (step === "screen_time_success") {
      return (
        <div className="text-center space-y-4 animate-fade-in">
          ...
          <p className="text-slate-500">
            Your screen time of{" "}
            <span className="font-bold text-navy-900">{inputValue} min</span> is
            well within your {habit.triggerConfig?.thresholdMinutes} min limit.
          </p>
    ...
    if (step === "location_success") {
      return (
        <div className="text-center space-y-4 animate-fade-in">
          ...
          <p className="text-slate-500">
            Location verified at <br />{" "}
            <span className="font-bold text-navy-900">
              {habit.triggerConfig?.locationName}
            </span>
          </p>
          <button
            onClick={() => onConfirm()}
        ...
        return (
          <div className="text-center space-y-4">
            ...
            <button
              ...
              className="w-full bg-navy-900 text-white py-3 rounded-xl font-medium mt-4 hover:bg-navy-800 transition-colors"
            >
              Arrive at Location
            </button>
          </div>
          ...
          <div className="text-center space-y-4">
            ...
            <button
              onClick={() => {
                if (
                  parseInt(inputValue) <
                  (habit.triggerConfig?.thresholdMinutes || 999)
                ) {
      ...
      case TriggerType.APP_OPEN:
          ...
          return (
            <div className="text-center space-y-4">
              ...
              <p className="text-slate-500 text-sm">
                {habit.triggerConfig.actionDetail} via{" "}
                {habit.triggerConfig.appName}
              </p>
        ...
        return (
          <div className="text-center space-y-4">
            ...
            <button
              onClick={handleAction}
              className="w-full bg-navy-900 text-white py-3 rounded-xl font-medium mt-4"
            >
              Launch App & Read
            </button>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in">
      <div className="bg-white dark:bg-dark-card rounded-[2.5rem] w-full max-w-sm p-8 shadow-2xl relative overflow-hidden border border-slate-100 dark:border-dark-border/50">
        <button
          onClick={onClose}
          className="absolute top-6 right-6 p-2 rounded-2xl text-slate-400 hover:bg-slate-50 dark:hover:bg-dark-cardHover transition-all"
        >
          <X size={20} />
        </button>
        {renderContent()}
