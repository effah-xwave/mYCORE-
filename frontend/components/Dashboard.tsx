...
const FocusAreaTag = ({
  ...
  >
    <div className="flex items-center gap-3">
      <div
        className={`p-2 rounded-lg ${active ? "bg-blue-500 text-white" : "bg-slate-100 dark:bg-dark-cardHover text-slate-400"}`}
      >
        <Icon size={16} />
      </div>
      <span
        className={`text-sm font-semibold ${active ? "text-blue-700 dark:text-blue-400" : "text-slate-500 dark:text-slate-400"}`}
      >
        {name}
      </span>
    </div>
    ...
    <div className="w-16 h-1 bg-slate-200 dark:bg-dark-border rounded-full overflow-hidden">
      <div
        className={`h-full rounded-full ${active ? "bg-blue-500 w-3/4" : "bg-slate-300 dark:bg-slate-600 w-1/4"}`}
      />
    </div>
  </div>
);
...
const DayCell = ({ date, isSelected, isToday, hasData, onClick }: any) => {
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-lg md:text-xl font-bold ${isSelected ? "text-white" : "text-slate-700 dark:text-slate-200"}`}
      >
        {dayNum}
      </span>
      {isToday && !isSelected && (
      ...
      {hasData && !isSelected && !isToday && (
        <div className="w-1 h-1 bg-slate-300 dark:bg-slate-600 rounded-full mt-1" />
      )}
    </button>
  );
};
  ...
  return (
    <div
      ...
      className={`
                ...
                ${
                  isCompleted
                    ? "bg-green-500/5 border-green-500/20 grayscale-[0.3] opacity-60"
                    : "bg-white dark:bg-dark-card shadow-soft border-slate-100 dark:border-dark-border/30 hover:border-slate-200 dark:hover:border-dark-border/80 hover:shadow-apple"
                }
            `}
    >
      <div className="flex items-center gap-4">
        ...
        <div>
          <h4
            className={`text-sm font-bold ${isCompleted ? "text-slate-400 line-through dark:text-slate-500" : "text-navy-900 dark:text-slate-200"}`}
          >
            {habit.name}
          </h4>
      ...
      >
        {isCompleted && (
          <Icons.Check size={14} className="text-white" strokeWidth={4} />
        )}
      </div>
    </div>
  );
...
export default function Dashboard() {
  ...
  const dayScore =
    totalCount > 0 ? Math.round((completedCount / totalCount) * 1000) : 0; // Score out of 1000 like credit score

  const mainTask =
    tasks.find((t) => t.priority === "High" && !t.completed) || tasks[0];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pb-8">
      ...
      <div className="lg:col-span-8 flex flex-col gap-6">
        ...
        <div className="bg-white dark:bg-dark-card rounded-[2rem] p-6 md:p-8 shadow-soft dark:shadow-dark-soft border border-slate-200/60 dark:border-dark-border/40 transition-colors">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-4 text-xs font-bold text-slate-400">
              <button className="hover:text-navy-900 dark:hover:text-white transition-colors">
                Last Week
              </button>
              <button className="text-navy-900 dark:text-white">
                Next Week
              </button>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center bg-slate-50 dark:bg-[#0B1221] p-2 rounded-[1.5rem]">
            {weekDays.map((d, i) => (
              <DayCell
                ...
                hasData={(currentWeekInstances[formatDate(d)] || []).some(
                  (x) => x.completed,
                )}
          ...
          <div className="mt-8 space-y-3">
            {dayInstances.slice(0, 2).map((inst) => {
              ...
              return (
                <div
                  key={inst.id}
                  className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-dark-cardHover/30 border border-slate-100 dark:border-dark-border/50"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Max Score Card */}
          <div className="bg-white dark:bg-dark-card rounded-[2rem] p-6 shadow-soft dark:shadow-dark-soft border border-slate-200/60 dark:border-dark-border/40 relative overflow-hidden group">
              ...
              <div className="w-20 h-20 relative flex items-center justify-center">
                <svg className="transform -rotate-90 w-full h-full">
                  ...
                  <circle
                    ...
                    strokeDasharray={200}
                    strokeDashoffset={
                      mainTask ? (mainTask.completed ? 0 : 50) : 0
                    }
                    strokeLinecap="round"
                    fill="transparent"
                    r="32"
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-[8px] uppercase font-bold text-green-400">
                    Excel
                  </span>
                </div>
              </div>
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-slate-400 text-sm">
                  Analysis of your goals prepared.
                </p>
              </div>
      ...
      <div className="lg:col-span-4 flex flex-col gap-6">
        {/* Focus Areas */}
        <div className="bg-white dark:bg-dark-card rounded-[2rem] p-6 shadow-soft dark:shadow-dark-soft border border-slate-200/60 dark:border-dark-border/40 h-fit">
          <h3 className="font-bold text-xl text-navy-900 dark:text-white mb-2">
            Focus areas
          </h3>
          <p className="text-xs text-slate-400 font-medium mb-6">
            Your targeted growth engines
          </p>
