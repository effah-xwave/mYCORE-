...
export default function OptimizeRoutineModal({ onClose, isPage = false }: OptimizeRoutineModalProps) {
  ...
  const content = (
    <motion.div 
      initial={isPage ? { opacity: 0, y: 20 } : { opacity: 0, scale: 0.9, y: 20 }}
      animate={{ opacity: 1, scale: 1, y: 0 }}
      className={`${isPage ? 'w-full' : 'bg-white dark:bg-dark-card w-full max-w-2xl rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-dark-border overflow-hidden flex flex-col max-h-[90vh]'}`}
    >
      {/* Header */}
      <div className={`p-8 border-b border-slate-100 dark:border-dark-border flex justify-between items-center ${isPage ? 'bg-transparent' : 'bg-slate-50/50 dark:bg-white/[0.02]'}`}>
      ...
      {/* Content */}
      <div className={`flex-1 overflow-y-auto p-8 custom-scrollbar ${isPage ? '' : ''}`}>
        {loading ? (
      ...
      {/* Footer */}
      <div className={`p-8 border-t border-slate-100 dark:border-dark-border ${isPage ? 'bg-transparent' : 'bg-slate-50/50 dark:bg-white/[0.02]'} flex justify-end gap-4`}>
        {!isPage && onClose && (
          <button 
            onClick={onClose}
        ...
        <button 
          onClick={() => isPage ? generateOptimization() : onClose?.()}
        ...
        >
          {isPage ? 'Refresh Suggestions' : 'Apply Suggestions'} <ArrowRight size={16} />
        </button>
      </div>
    </motion.div>
  );
  ...
  if (isPage) {
    return (
      <div className="bg-white dark:bg-dark-card rounded-[2.5rem] shadow-apple border border-slate-200 dark:border-dark-border overflow-hidden animate-fade-in">
        {content}
      </div>
    );
  }
