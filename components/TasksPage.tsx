...
const CalendarView = ({ tasks, onDateSelect, selectedDate }: { tasks: Task[], onDateSelect: (d: string) => void, selectedDate: string }) => {
  ...
  return (
    <div className="bg-slate-200/60 dark:bg-dark-card backdrop-blur-xl p-6 rounded-[2.5rem] shadow-apple border border-slate-300/50 dark:border-dark-border">
  ...
  return (
    <div className="space-y-8 pb-20 relative min-h-[80vh]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex bg-slate-200/40 dark:bg-dark-card/50 backdrop-blur-sm p-1 rounded-2xl shadow-sm border border-slate-300/50 dark:border-dark-border transition-all w-fit self-end md:self-auto">
           <button onClick={() => setView('list')} className={`p-2.5 rounded-xl transition-all ${view === 'list' ? 'bg-slate-200 dark:bg-dark-card text-slate-900 dark:text-white shadow-sm' : 'text-slate-400'}`}>
              <List size={20} />
           </button>
           <button onClick={() => setView('calendar')} className={`p-2.5 rounded-xl transition-all ${view === 'calendar' ? 'bg-slate-200 dark:bg-dark-card text-slate-900 dark:text-white shadow-sm' : 'text-slate-400'}`}>
      ...
      <div className={`grid grid-cols-1 ${view === 'calendar' ? 'lg:grid-cols-2' : ''} gap-8`}>
         ...
         <div className="bg-slate-200/60 dark:bg-dark-card backdrop-blur-xl rounded-[2.5rem] p-8 shadow-apple border border-slate-300/50 dark:border-dark-border min-h-[500px] relative animate-fade-in-up flex flex-col">
            ...
            {/* FILTER PANEL */}
            {showFilters && (
                <div className="flex flex-wrap gap-3 mb-6 animate-fade-in p-4 bg-slate-200/40 dark:bg-dark-bg/50 backdrop-blur-md rounded-2xl border border-slate-300/50 dark:border-dark-border shadow-inner">
                    {/* Project */}
                    <div className="relative">
                        <select 
                            value={filterProject} 
                            onChange={(e) => setFilterProject(e.target.value)}
                            className="appearance-none pl-9 pr-8 py-2 bg-slate-200 dark:bg-dark-card border border-slate-300 dark:border-dark-border rounded-xl text-xs font-bold text-slate-600 dark:text-slate-300 focus:outline-none focus:border-slate-900 dark:focus:border-blue-500 focus:ring-1 focus:ring-slate-900/10 cursor-pointer min-w-[140px]"
                        >
                    ...
                    {activeFiltersCount > 0 && (
                        <button 
                            ...
                            className="flex items-center gap-1.5 px-3 py-2 bg-slate-200 dark:bg-dark-card border border-slate-300 dark:border-dark-border hover:bg-red-100 dark:hover:bg-red-900/20 hover:border-red-200 hover:text-red-500 rounded-xl text-xs font-bold text-slate-500 transition-all ml-auto"
                        >
                            <X size={14} /> Clear
                        </button>
                    )}
